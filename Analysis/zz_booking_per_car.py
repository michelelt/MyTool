...
def plot_bookig_per_car_cdf(df1, provider, column, path):
    ...
    title = provider + " - Bookings per car"
    xlabel = "Bookings per car"
    
    df1 = pd.DataFrame(df1.groupby("plate").count()["city"]).rename(columns = {"city":"bookings_per_car"})
    df1 = df1[df1[ "bookings_per_car"] >= 61]
...
column = "bookings_per_car"

provider = 'enjoy'
path = "/home/mc/Scrivania/Tesi/Writing/figures/"+dir_+"/"+provider+"_"+column+"_"+name
plot_bookig_per_car_cdf(enj_bookings, "enjoy", column, path)


provider = 'car2go'
path = "/home/mc/Scrivania/Tesi/Writing/figures/"+dir_+"/"+provider+"_"+column+"_"+name
plot_bookig_per_car_cdf(c2g_bookings, "car2go", column, path)
#
